...
/// <summary>
/// Kimi 重试 HTTP 消息处理器，用于处理 Kimi 请求的重试逻辑
/// </summary>
public class KimiRetryHttpMessageHandler : DelegatingHandler
{
    /// <summary>
    /// Kimi 配置选项
    /// </summary>
    ...
    /// 日志记录器
    /// </summary>
    private readonly ILogger<KimiRetryHttpMessageHandler>? _logger;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="options">Kimi 配置选项</param>
    /// <param name="logger">日志记录器</param>
    public KimiRetryHttpMessageHandler(KimiOptions options, ILogger<KimiRetryHttpMessageHandler>? logger = null)
    {
        _options = options;
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            {
                // 设置BaseAddress
                var baseUrl = _options.BaseUrl ?? "https://api.moonshot.cn/v1/";
                if (!currentRequest.RequestUri!.IsAbsoluteUri)
                {
                    currentRequest.RequestUri = new Uri(new Uri(baseUrl), currentRequest.RequestUri);
                }
