...
/// <summary>
/// ERNIE 重试 HTTP 消息处理器，用于处理 ERNIE 请求的重试逻辑
/// </summary>
public class ERNIERetryHttpMessageHandler : DelegatingHandler
{
    /// <summary>
    /// ERNIE 配置选项
    /// </summary>
    ...
    /// 日志记录器
    /// </summary>
    private readonly ILogger<ERNIERetryHttpMessageHandler>? _logger;
    ...
    public ERNIERetryHttpMessageHandler(ERNIEOptions options, ILogger<ERNIERetryHttpMessageHandler>? logger = null)
    {
        _options = options;
        _logger = logger;
    }
        ...
        while (true)
        {
            ...
            try
            {
                ...
                if (IsTransientFailure(response.StatusCode) && retryAttempt < _options.MaxRetryCount)
                {
                    ...
                    _logger?.LogWarning("ERNIE request failed with status code {StatusCode}. Retrying attempt {RetryAttempt}/{MaxRetryCount} after {Delay}ms",
            ...
            catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
            {
                if (retryAttempt < _options.MaxRetryCount)
                {
                    retryAttempt++;
                    var delay = CalculateDelay(retryAttempt);
                    _logger?.LogWarning("ERNIE request timed out. Retrying attempt {RetryAttempt}/{MaxRetryCount} after {Delay}ms",
                        retryAttempt, _options.MaxRetryCount, delay.TotalMilliseconds);
                    await Task.Delay(delay, cancellationToken);
                    continue;
                }

                _logger?.LogError("ERNIE request timed out after {MaxRetryCount} retries", _options.MaxRetryCount);
                throw;
            }
            catch (HttpRequestException ex)
            {
                if (retryAttempt < _options.MaxRetryCount)
                {
                    retryAttempt++;
                    var delay = CalculateDelay(retryAttempt);
                    _logger?.LogWarning(ex, "ERNIE network error. Retrying attempt {RetryAttempt}/{MaxRetryCount} after {Delay}ms",
                        retryAttempt, _options.MaxRetryCount, delay.TotalMilliseconds);
                    await Task.Delay(delay, cancellationToken);
                    continue;
                }

                _logger?.LogError(ex, "ERNIE network error after {MaxRetryCount} retries", _options.MaxRetryCount);
                throw;
            }
        }
    }
