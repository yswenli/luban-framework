...
/// <summary>
/// MiMo 聊天模型提供者，用于与 MiMo API 进行交互
/// </summary>
public class MiMoChatModelProvider : IChatModelProvider
{
    /// <summary>
    /// 提供者名称
    /// </summary>
    public string ProviderName => "mimo";
    ...
    /// 日志记录器
    /// </summary>
    private readonly ILogger<MiMoChatModelProvider>? _logger;
    
    /// <summary>
    /// MiMo 配置选项
    /// </summary>
    private readonly MiMoOptions _options;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="httpClient">HTTP 客户端</param>
    /// <param name="options">MiMo 配置选项</param>
    /// <param name="logger">日志记录器</param>
    public MiMoChatModelProvider(
        HttpClient httpClient,
        Microsoft.Extensions.Options.IOptions<MiMoOptions> options,
        ILogger<MiMoChatModelProvider>? logger = null)
    {
    ...
    {
        return _options.BaseUrl ?? "https://api.xiaomimimo.com/anthropic/";
    }
    ...
    public async Task<ChatResponse> CompleteAsync(ChatRequest request, CancellationToken cancellationToken = default)
    {
        ...
        _logger?.LogInformation("Starting MiMo request {RequestId} to model {ModelId}", requestId, request.ModelId);
        ...
        try
        {
            ...
            var providerResponse = JsonSerializer.Deserialize<MiMoMessagesResponse>(responseJson, _jsonOptions);
            _logger?.LogInformation("MiMo request {RequestId} completed successfully", requestId);
            return MapFromResponse(providerResponse);
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "MiMo request {RequestId} failed", requestId);
    ...
    {
        var requestId = Guid.NewGuid().ToString();
        _logger?.LogInformation("Starting streaming MiMo request {RequestId} to model {ModelId}", requestId, request.ModelId);
        ...
        {
            _logger?.LogError(ex, "Streaming MiMo request {RequestId} failed during initialization", requestId);
            initialException = ex;
        }
        ...
        while ((line = await reader.ReadLineAsync(cancellationToken)) != null)
        {
            ...
            MiMoMessagesStreamEvent? streamEvent;
            try
            {
                streamEvent = JsonSerializer.Deserialize<MiMoMessagesStreamEvent>(line, _jsonOptions);
            }
        ...
        _logger?.LogInformation("Streaming MiMo request {RequestId} completed", requestId);
    }
    ...
    /// <returns>MiMo 消息请求</returns>
    private MiMoMessagesRequest CreateProviderRequest(ChatRequest request, bool stream)
    {
        ...
        var providerRequest = new MiMoMessagesRequest
        {
            Model = request.ModelId.StartsWith("mimo:") ? request.ModelId.Substring("mimo:".Length) : request.ModelId,
            Stream = stream,
            Messages = messages,
        ...
        {
            providerRequest.Tools = request.Options.Tools.Select(t => new MiMoToolDefinition
            {
                Name = t.Name,
    ...
    /// <returns>MiMo 消息</returns>
    private MiMoMessage MapToMessage(ChatMessage message)
    {
        ...
        var content = new List<MiMoContentBlock>();

        if (!string.IsNullOrEmpty(message.TextContent))
        {
            if (message.Role == ChatRole.Tool)
            {
                content.Add(new MiMoContentBlock
                {
                    Type = "tool_result",
    ...
    /// 从 MiMo 响应映射为聊天响应
    /// </summary>
    /// <param name="response">MiMo 消息响应</param>
    /// <returns>聊天响应</returns>
    private ChatResponse MapFromResponse(MiMoMessagesResponse? response)
    {
        if (response == null)
        {
            return new ChatResponse { IsSuccess = false, ErrorMessage = "Invalid response from MiMo" };
        }
...
/// MiMo 消息请求
/// </summary>
public class MiMoMessagesRequest
{
    /// <summary>
    /// 模型名称
    /// </summary>
    ...
    /// 消息列表
    /// </summary>
    public List<MiMoMessage> Messages { get; set; } = [];
    ...
    /// 工具定义列表
    /// </summary>
    public List<MiMoToolDefinition>? Tools { get; set; }
}
...
public class MiMoMessage
{
    ...
    /// 内容块列表
    /// </summary>
    public List<MiMoContentBlock> Content { get; set; } = [];
}

/// <summary>
/// MiMo 内容块
/// </summary>
public class MiMoContentBlock
{
    /// <summary>
    /// 类型
    /// </summary>
...
/// MiMo 工具定义
/// </summary>
public class MiMoToolDefinition
{
    /// <summary>
    /// 工具名称
    /// </summary>
...
/// MiMo 消息响应
/// </summary>
public class MiMoMessagesResponse
{
    /// <summary>
    /// 内容块列表
    /// </summary>
...
/// MiMo 消息流事件
/// </summary>
public class MiMoMessagesStreamEvent
{
    /// <summary>
    /// 事件类型
    /// </summary>
    ...
    /// 内容块
    /// </summary>
    public MiMoContentBlock? ContentBlock { get; set; }

    /// <summary>
    /// 增量
    /// </summary>
    public MiMoContentBlockDelta? Delta { get; set; }
...
/// MiMo 内容块增量
/// </summary>
public class MiMoContentBlockDelta
{
    /// <summary>
    /// 文本增量
    /// </summary>
