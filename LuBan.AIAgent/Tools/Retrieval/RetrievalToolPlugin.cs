...
*机器名称：WALLE
*公司名称：Walle
*命名空间：LuBan.AIAgent.Tools.Retrieval
*文件名： RetrievalToolPlugin
*版本号： V1.0.0.0
*唯一标识：cde77c24-d326-47bc-ac75-557ebec5b92b
*当前的用户域：WALLE
*创建人： yswenli
...
public class RetrievalToolPlugin : ILuBanToolPlugin
{
    ...
    public IReadOnlyList<AIFunction> GetTools(IServiceProvider sp)
    {
        ...
        return new List<AIFunction>
        {
            ...
            AIFunctionFactoryHelper.Create(group, nameof(RetrievalToolGroup.IndexContentAsync)),
            AIFunctionFactoryHelper.Create(group, nameof(RetrievalToolGroup.SearchCodeAsync)),
            AIFunctionFactoryHelper.Create(group, nameof(RetrievalToolGroup.GetIndexStatsAsync))
        };
    }
...
public class RetrievalToolGroup
{
    ...
    public async Task<ToolResult<string>> IndexDirectoryAsync(
        ...
        [Description("强制全部重建索引")] bool force = false)
    {
        ...
        catch (Exception ex)
        {
            ...
            return ToolResult.Fail<string>($"错误：索引目录失败 - {ex.Message}");
        }
    }
    ...
    public async Task<ToolResult<string>> IndexContentAsync(
        ...
        [Description("来源标识，如 web://example.com/page")] string sourceName)
    {
        ...
        try
        {
            ...
            return ToolResult.Ok<string>($"索引完成：{sourceName}，切块 {r.TotalChunks}（新嵌入 {r.EmbeddedChunks}）");
        }
        catch (Exception ex)
        {
            Logger.Error("索引内容失败", ex, sourceName);
            return ToolResult.Fail<string>($"错误：索引内容失败 - {ex.Message}");
        }
    }
    ...
    public async Task<ToolResult<string>> SearchCodeAsync(
        ...
        [Description("限定语言（可选），如 csharp、html")] string? language = null)
    {
        try
        {
            ...
            if (results.Count == 0) return ToolResult.Ok<string>("未找到相关内容。请先用 IndexDirectoryAsync 或 IndexContentAsync 建立索引。");
            var sb = new StringBuilder();
        ...
        catch (Exception ex)
        {
            ...
            return ToolResult.Fail<string>($"错误：语义检索失败 - {ex.Message}");
        }
    }
    ...
    public async Task<ToolResult<string>> GetIndexStatsAsync()
    {
        try
        {
            var s = await _service.GetStatsAsync();
            return ToolResult.Ok<string>($"已索引文件 {s.TotalFiles} 个，切块 {s.TotalChunks} 个，模型 {s.ModelId ?? "未知"}，向量维度 {s.VectorDimension}");
        }
        catch (Exception ex)
        {
            Logger.Error("获取索引统计失败", ex);
            return ToolResult.Fail<string>($"错误：获取索引统计失败 - {ex.Message}");
        }
    }
}
