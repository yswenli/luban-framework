...
public class RedisToolGroup
{
    ...
    public RedisToolGroup(RedisToolOptions options, ProcessRunner processRunner, IToolConfirmationService confirmationService)
    {
        _options = options;
        _processRunner = processRunner;
    ...
    public async Task<ToolResult<string>> ExecAsync(string command)
    {
        ...
        {
            var outcome = await _confirmationService.EvaluateAsync(nameof(ExecAsync), null,
