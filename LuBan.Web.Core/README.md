...
- **内部服务接口认证**：`[InternalApiAccess]` 只需传入 Bearer {RefreshToken}，供内部服务间调用，无需走完整 AccessToken 流程
...
4. **内部服务接口**：使用 `[InternalApiAccess]` 标签时，只需传入 `Bearer {RefreshToken}`，无需获取 AccessToken
