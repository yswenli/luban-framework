...
public static class DictionaryUtil
{
    ...
    /// <summary>
    /// 捕获多个变量【变量名‑字符串值】转为 Dictionary&lt;string,string&gt;
    /// <example>
    /// var dict = DictionaryUtil.CaptureDictionary(()=>id,()=>name,()=>model);
    /// </example>
    /// </summary>
    /// <param name="captures">lambda表达式 ()=>变量</param>
    /// <returns>永远不返回null；无输入返回空字典</returns>
    public static Dictionary<string, string> CaptureDictionary(params Expression<Func<object?>>[] captures)
    {
        var dict = new Dictionary<string, string>();
        if (captures == null || captures.Length == 0)
            return dict;

        foreach (var exp in captures)
        {
            string key;
            object? value;
            try
            {
                ...
                {
                    key = memberExp.Member.Name;
                }
                else if (exp.Body is UnaryExpression unary && unary.Operand is MemberExpression innerMember)
                {
                    key = innerMember.Member.Name;
                }
                else
                {
                    // 无法解析为成员变量，例如 ()=>123 字面量，取表达式描述
                    key = exp.Body.ToString();
                }
            ...
            {
                strVal = value.ToString() ?? string.Empty;
            }
            else
            {
                // 引用类型对象走序列化，加异常保护防止循环引用崩溃
                try
                {
                    strVal = SerializeUtil.Serialize(value);
                }
                catch
                {
                    strVal = value.ToString() ?? string.Empty;
