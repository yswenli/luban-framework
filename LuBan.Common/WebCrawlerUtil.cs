...
*文件名： WebCrawlerUtil
*版本号： V1.0.0.0
*唯一标识：1531fafe-71ca-454d-a903-3acc4d0ff4b3
*当前的用户域：WALLE
*创建人： WALLE
*电子邮箱：yswenli@outlook.com
*创建时间：2023/2/14 17:11:58
*描述：爬虫工具类
*
*=================================================
*修改标记
*修改时间：2023/2/14 17:11:58
*修改人： yswen
*版本号： V1.0.0.0
*描述：爬虫工具类
*
...
namespace LuBan.Common
{
    ...
    private static int currentTry = 0;
        ...
        public static async Task<string> GetHtmlAsync(string url, string postData, bool isPost, CookieContainer cookieContainer)
        {
            if (string.IsNullOrEmpty(postData))
                return await GetHtmlAsync(url, cookieContainer);
            ...
            try
            {
                ...
                var client = HttpClientProxy.Create(baseUri, timeout: 19, cookiescontainer: cookieContainer);
                ...
                if (isPost)
                {
                    ...
                    return await client.PostAsync(relativePath, postData, headers);
                }
                else
                {
        ...
        /// <param name="cookieContainer">CookieContainer</param>
        /// <returns>HTML内容</returns>
        public static async Task<string> GetHtmlAsync(string url, CookieContainer cookieContainer)
        {
            try
            {
                var uri = new Uri(url);
        ...
        public static async Task<Stream?> GetStreamAsync(string url, CookieContainer cookieContainer)
        {
            try
            {
                var uri = new Uri(url);
                ...
                var client = HttpClientProxy.Create(baseUri, timeout: 19, cookiescontainer: cookieContainer);
                var bytes = await client.GetBytesAsync(relativePath);
        ...
        /// 以GET方式抓取远程页面内容（异步版本）
        /// </summary>
        /// <param name="tUrl">目标URL</param>
        /// <returns>页面内容</returns>
        public static async Task<string> Get_HttpAsync(string tUrl)
        {
            try
            {
                var uri = new Uri(tUrl);
                var baseUri = new Uri(uri.GetLeftPart(UriPartial.Authority));
                var relativePath = uri.PathAndQuery;

                var client = HttpClientProxy.Create(baseUri, timeout: 19);
                return await client.GetAsync(relativePath);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// 以POST方式抓取远程页面内容（异步版本）
        /// </summary>
        /// <param name="url">目标URL</param>
        /// <param name="postData">POST数据</param>
        /// <param name="encodeType">编码类型</param>
        /// <returns>页面内容</returns>
        public static async Task<string> Post_HttpAsync(string url, string postData, string encodeType)
        {
            try
            {
                var uri = new Uri(url);
                ...
                var headers = new Dictionary<string, string>
                {
                    { "Content-Type", $"application/x-www-form-urlencoded; charset={encodeType}" }
                };
